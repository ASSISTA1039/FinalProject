...
public class GrapplingRope : MonoBehaviour
{
    ...
    public AnimationCurve ropeReleasedCurve;
    [Range(0.01f, 4)] [SerializeField] private float StartWaveSize = 3;
    float waveSize = 0;

    [Header("GrapplingRope Recycling")]
    public AnimationCurve ropeRecyclingCurve;
    [SerializeField] [Range(1, 50)] private float ropeRecyclingSpeed = 1.25f;
    ...
    void DrawRopeWaves()    //Adjustment of ripple deviation
    {
        for (int i = 0; i < percision; i++)
        {
            ...
            offset = Vector2.Perpendicular(grapplingGun.grappleDistanceVector).normalized * ropeReleasedCurve.Evaluate(delta) * waveSize;
            ...
            currentPosition = Vector2.Lerp(grapplingGun.firePoint.position, targetPosition, ropeRecyclingCurve.Evaluate(moveTime) * ropeRecyclingSpeed);

            m_lineRenderer.SetPosition(i, currentPosition);
        }
        if (grapplingGun.touchGuandao)
            openGuandao = true;
    }
}
